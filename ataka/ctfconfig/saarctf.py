...
ATAKA_HOST = 'ataka.ad.mnta.in'

# Default targets for atk runlocal
RUNLOCAL_TARGETS = ["10.33.1.2"]

# IPs that are always excluded from attacks.
STATIC_EXCLUSIONS = {'10.33.89.2'}
...
START_TIME = 1762610400 # Fri May 20 2022 4:00:01 PM GMT+02:00 (Central European Summer Time)
...
SUBMIT_PORT = 31337
FLAGID_URL = 'https://scoreboard.ctf.saarland/api/attack.json'


def get_services():
    print("Fetching services from SaarCTF config...")
...
def submit_flags(flags):
    ...
    try:
        ...
        sock.settimeout(2)
        sock.connect((SUBMIT_DOM, SUBMIT_PORT))

        for flag in flags:
            sock.sendall((flag + '\n').encode())
            resp = sock.recv(100000).decode()
            if resp == '[OK]\n':
